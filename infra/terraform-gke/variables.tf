...
# ---- Application secrets (seeded into GCP Secret Manager by Terraform) ----
# Set these in terraform.tfvars. ESO syncs them into Kubernetes Secrets at runtime.

variable "postgres_password" {
  description = "PostgreSQL password — seeded into Secret Manager as 'jerney-postgres-password'"
  type        = string
  sensitive   = true
  nullable    = false
}
...
variable "grafana_admin_password" {
  description = "Grafana admin password — seeded into Secret Manager as 'jerney-grafana-admin-password'"
  type        = string
  sensitive   = true
  nullable    = false
}
...
variable "alertmanager_smtp_key" {
  description = "Resend SMTP API key — seeded into Secret Manager as 'jerney-alertmanager-smtp-key'"
  type        = string
  sensitive   = true
  nullable    = false
}
